...
def main():
    ...
    precise_val = scipy.integrate.quad(phi, left, right)[0]
    print(f"Точное значение интеграла от {left} до {right} от функции sin(x) * x ^ (1 / 4): {precise_val:.12f}")
    # calc_val = 
    num_nodes = int(input("Введите количество узлов ИКФ: "))
    while True:
        nodes: List[float] = list(map(float, input(f"Введите {num_nodes} различных узлов ИКФ через пробел: ").split()))
        if len(list(set(nodes))) != len(nodes) or len(nodes) != num_nodes:
